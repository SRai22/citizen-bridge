...
def upgrade() -> None:
    op.create_table(
        ...
        sa.ForeignKeyConstraint(["source_case_id"], ["cases.cases.id"], ondelete="CASCADE"),
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint("source_case_id"),
        sa.UniqueConstraint("user_id", "benefit_id", name="uq_active_benefit_user_scheme"),
        schema="cases",
    )
    op.create_index(
        "ix_cases_active_benefits_user_id", "active_benefits", ["user_id"], schema="cases"
    )
    op.create_index(
        "ix_cases_active_benefits_benefit_id", "active_benefits", ["benefit_id"], schema="cases"
    )
    op.create_index(
        "ix_cases_active_benefits_status", "active_benefits", ["status"], schema="cases"
    )
    op.create_table(
        "benefit_discoveries",
        sa.Column("user_id", sa.Uuid(), nullable=False),
        ...
        sa.Column("updated_at", sa.DateTime(timezone=True), nullable=False),
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint("user_id", "benefit_id", name="uq_benefit_discovery_user_scheme"),
        schema="cases",
    )
    op.create_index(
